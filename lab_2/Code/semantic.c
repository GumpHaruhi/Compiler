...
void freeType(Type type, Standard standard){
    if(type == NULL) { return; }
    if(type->kind == ARRAY){
        freeType(type->array.elem, standard);
    } else if(type->kind == STRUCTURE){
        if(standard == STRICT){
            freeField(type->structure->members, standard);
            free(type->structure); type->structure = NULL;
        } else if(strcmp(type->structure->name, "") == 0){
            freeField(type->structure->members, standard);
            free(type->structure); type->structure = NULL;
        }
    } else if(type->kind == FUNCTION){
        freeType(type->func->ret_type, standard);
        freeField(type->func->param, standard);
        free(type->func); type->func = NULL;
...
void freeField(FieldList list, Standard standard){
    if(list == NULL) { return; }
    freeField(list->next, standard);
    freeType(list->type, standard);
    if(list){
        free(list);
        list = NULL;
    }   
    ...
    if(node->childnum == 2){
        ...
        if(strcmp(str, "") != 0){
            ...
            if(sym != NULL){
                ...
                freeType(type, RELAXED);  free(str); str = NULL;
                return NULL;
            }
            // 定义一个新结构体
